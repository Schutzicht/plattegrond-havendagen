...
const fsLabel = fsBtn.querySelector('.fullscreen-btn-label');
const mapScroll = document.getElementById('map-scroll');

function toggleFullscreen() {
  const isOn = document.body.classList.toggle('is-fullscreen');
  fsBtn.setAttribute('aria-label', isOn ? 'Sluit volledig scherm' : 'Volledig scherm');
  if (fsLabel) fsLabel.textContent = isOn ? 'Sluiten' : 'Volledig scherm';
  // Replace icon: enter <-> exit
  fsBtn.querySelector('svg').innerHTML = isOn
    ? '<path d="M9 4v5H4M15 4v5h5M9 20v-5H4M15 20v-5h5"/>'   // exit (corners pointing inward)
    : '<path d="M4 9V4h5M20 9V4h-5M4 15v5h5M20 15v5h-5"/>'; // enter (corners pointing outward)
  // Prevent body scroll while in fullscreen
  document.body.style.overflow = isOn ? 'hidden' : '';
  // Auto-scroll the map to start position when entering fullscreen
  if (isOn) mapScroll.scrollLeft = 0;
}
